...
export async function getPublicCatalogContent(locale: Locale) {
  ...
  try {
    ...
    const [destinationsResult, servicesResult, promotionsResult] = await Promise.allSettled([
    ...
    return buildPublicCatalogContent(locale, {
      destinations: destinationsResult.status === "fulfilled" ? (destinationsResult.value.data ?? []) as CatalogRowLike[] : [],
      services: servicesResult.status === "fulfilled" ? (servicesResult.value.data ?? []) as CatalogRowLike[] : [],
      promotions: promotionsResult.status === "fulfilled" ? (promotionsResult.value.data ?? []) as CatalogRowLike[] : [],
    });
  } catch {
...
export async function getPublicCatalogItem(locale: Locale, kind: "destinations" | "promotions", slug: string) {
  ...
  const item = (kind === "promotions" ? catalog?.promotions : catalog?.destinations)?.find((entry) => entry.slug[locale] === slug)
    ?? (kind === "promotions" ? getPublicSiteContent(locale).promotions : getPublicSiteContent(locale).destinations).find((entry) => entry.slug[locale] === slug);
  return item ?? null;
}
