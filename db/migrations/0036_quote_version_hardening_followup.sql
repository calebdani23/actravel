-- Follow-up hardening for already-deployed quote version environments.

alter table public.quote_versions
  add column if not exists idempotency_key text;

drop index if exists public.quote_versions_lead_idempotency_key_idx;
...
alter table public.quote_versions
  drop constraint if exists quote_versions_lead_id_fkey,
  add constraint quote_versions_lead_id_fkey
    foreign key (lead_id) references public.leads(id) on delete restrict;

alter table public.quote_versions
  drop constraint if exists quote_versions_contact_id_fkey,
  add constraint quote_versions_contact_id_fkey
    foreign key (contact_id) references public.contacts(id) on delete restrict;

drop function if exists public.crm_quote_version_relationship_override_enabled();
