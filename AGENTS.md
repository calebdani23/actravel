...
- This repo currently has no app manifest or test/build config; `README.md` is only a title, so treat the checked-in surface as repo tooling rather than an application.
...
- Use `scripts/opencode-portfolio` to run repo-local OpenCode: it sources `scripts/ai-env.sh`, prepends `.opencode-bin`/`.gentle-ai-bin` to `PATH`, sets `XDG_*` under `.opencode-runtime`, and execs `.opencode-bin/opencode`.
...
- `.opencode/package.json` only installs `@opencode-ai/plugin@1.15.0`; no root package manager lockfile or install command exists for an application yet.
