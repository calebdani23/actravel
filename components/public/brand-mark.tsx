...
export function BrandMark({ locale }: Readonly<{ locale: Locale }>) {
  return (
    <Link href={`/${locale}`} className="group inline-flex items-center gap-3" aria-label="AC Travel home">
      <span className="grid size-11 place-items-center rounded-2xl bg-[var(--ac-orange)] font-black text-white shadow-sm shadow-orange-900/10 group-hover:scale-[1.03]">
        AC
      </span>
